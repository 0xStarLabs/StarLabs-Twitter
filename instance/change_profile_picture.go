...
// ChangeProfilePicture change account profile picture
func (twitter *Twitter) ChangeProfilePicture(pictureBase64Encoded string) bool {
	pictureURL := fmt.Sprintf("https://twitter.com/i/api/1.1/account/update_profile_image.json")
	...
	for i := 0; i < twitter.config.Info.MaxTasksRetries; i++ {
		...
		if err != nil {
			twitter.logger.Error("Failed to build change profile picture request: %s", err.Error())
			continue
		}
		...
		if err != nil {
			twitter.logger.Error("Failed to do change profile picture request: %s", err.Error())
			continue
		}
		...
		if resp.StatusCode >= 200 && resp.StatusCode <= 299 {
			if strings.Contains(bodyString, "screen_name") {
				twitter.logger.Success("%d | %s successfully changed profile picture", twitter.index, twitter.Username)
				return true
			}
			...
			twitter.logger.Error("%d | Unknown response while change profile picture: %s", twitter.index, bodyString)
			continue
		}
	}

	extra.Logger{}.Error("%d | Unable to change profile picture", twitter.index)
	return false
}
