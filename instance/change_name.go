...
func (twitter *Twitter) ChangeName(newName string) bool {
	changeNameURL := fmt.Sprintf("https://twitter.com/i/api/1.1/account/update_profile.json")

	for i := 0; i < twitter.config.Info.MaxTasksRetries; i++ {
		var stringData = fmt.Sprintf(`name=%s`, newName)
		data := strings.NewReader(stringData)

		// Create new request
		req, err := http.NewRequest("POST", changeNameURL, data)
		if err != nil {
			extra.Logger{}.Error("%d | Failed to build change name request: %s", err.Error())
			continue
		}
		...
		if err != nil {
			extra.Logger{}.Error("%d | Failed to do change name request: %s", err.Error())
			continue
		}
		...
		case "gzip":
			...
			if err != nil {
				extra.Logger{}.Error("%d | Failed to create gzip reader while change name: %s", err.Error())
				continue
			}
			defer reader.Close()
		default:
		...
		if err != nil {
			extra.Logger{}.Error("%d | Failed to read change name response body: %s", err.Error())
			continue
		}
		...
		if resp.StatusCode >= 200 && resp.StatusCode <= 299 {
			if strings.Contains(bodyString, "name") {
				...
				err = json.Unmarshal(bodyBytes, &changeNameResponse)
				if err != nil {
					extra.Logger{}.Error("%d | Failed to do unmarshal in change name response: %s", twitter.index, err.Error())
					continue
				}
				if changeNameResponse.Name == newName {
					twitter.logger.Success("%d | %s | Successfully changed name", twitter.index, twitter.Username)
					return true
				}
			}
			...
			extra.Logger{}.Error("%d | Unknown response while change name: %s", twitter.index, bodyString)
			continue
		}
	}

	extra.Logger{}.Error("%d | %s | Unable to change name", twitter.index, twitter.Username)
	return false
}
